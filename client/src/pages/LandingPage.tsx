    ...
    <div className={`landing-root ${mounted ? 'is-mounted' : ''}`}>
      <style>{`
        ...
        :root {
          ...
          --gold-dim: rgba(201,168,76,0.35); --white: #F7F2EA;
          --white-dim: rgba(247,242,234,0.75); --bg-deep: #05030E;
          --bg-mid: #0B0618; --purple-dim: rgba(120,80,200,0.12);
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .landing-root {
          ...
          background: var(--bg-deep); font-family: 'Jost', sans-serif;
        ...
        .bg-lines {
          position: absolute; inset: 0;
          background-image: repeating-linear-gradient(0deg, transparent, transparent 79px, rgba(255,255,255,0.012) 80px);
          pointer-events: none;
        }
        ...
        .ring-1 { inset: 20px; border-color: rgba(201,168,76,0.18) transparent rgba(201,168,76,0.06) transparent; animation: spin 35s linear infinite; }
        .ring-2 { inset: 35px; border-color: transparent rgba(180,140,255,0.15) transparent rgba(180,140,255,0.08); animation: spin 22s linear infinite reverse; }
        .ring-3 { inset: 50px; border-color: rgba(201,168,76,0.1) transparent rgba(201,168,76,0.04) transparent; animation: spin 48s linear infinite; }
        @keyframes spin { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }

        .orb-dot { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; }
        .orb-dot-inner {
          width: 48px; height: 48px; border-radius: 50%;
          background: radial-gradient(ellipse, rgba(240,208,128,0.9) 0%, rgba(201,168,76,0.6) 40%, rgba(140,90,200,0.2) 100%);
          box-shadow: 0 0 20px rgba(201,168,76,0.6), 0 0 60px rgba(201,168,76,0.2), 0 0 120px rgba(201,168,76,0.08);
        ...
        /* ✅ Header — toujours visible, z-index élevé */
        .header-bar {
        ...
        .is-mounted .header-bar { opacity: 1; transition-delay: 0.2s; }

        /* Contenu */
        .main-content {
          flex: 1; display: flex; flex-direction: column; align-items: center;
          justify-content: center; padding: 0 32px;
          position: relative; z-index: 10; width: 100%; gap: 0;
        }
        ...
        .main-title {
          ...
          font-size: clamp(42px, 11vw, 64px); font-weight: 300;
        ...
        .main-tagline {
          font-size: 11px; font-weight: 200; letter-spacing: 5px; text-transform: uppercase;
          color: rgba(247,242,234,0.55); margin-bottom: 36px;
          opacity: 0; transition: opacity 0.7s ease;
        }
        .is-mounted .main-tagline { opacity: 1; transition-delay: 0.5s; }

        .main-desc {
          font-family: 'Playfair Display', serif; font-size: 15px; font-style: italic; font-weight: 300;
          color: rgba(247,242,234,0.82); text-align: center; line-height: 1.75;
        ...
        .cta-btn:hover:not(:disabled) { border-color: rgba(201,168,76,0.75); color: var(--gold-light); box-shadow: 0 0 30px rgba(201,168,76,0.15), 0 8px 32px rgba(0,0,0,0.4); transform: translateY(-1px); }
        ...
        .footer-note {
          font-size: 11px; font-weight: 300; color: #F7F2EA; text-align: center;
          letter-spacing: 0.5px; opacity: 0; transition: opacity 0.7s ease;
        }
        .is-mounted .footer-note { opacity: 1; transition-delay: 0.6s; }

        .footer-bar { width: 100%; padding: 24px 32px; display: flex; justify-content: center; position: relative; z-index: 10; }

        /* Particules */
        .particles { position: absolute; inset: 0; pointer-events: none; overflow: hidden; }
        .particle { position: absolute; width: 1px; height: 1px; background: white; border-radius: 50%; animation: particle-fade var(--dur, 4s) ease-in-out infinite var(--del, 0s); }
        ...
        .overlay {
          ...
          background: rgba(5,3,14,0.9); backdrop-filter: blur(12px);
        ...
        .disc-text {
          ...
          color: rgba(247,242,234,0.75); line-height: 1.8; margin-bottom: 32px;
        }
        .disc-accept {
        ...
        .disc-accept:hover { background: linear-gradient(135deg, rgba(201,168,76,0.22) 0%, rgba(201,168,76,0.12) 100%); border-color: rgba(201,168,76,0.7); color: var(--gold-light); }
      `}</style>
      ...
      <div className="particles">
        {Array.from({ length: 50 }).map((_, i) => (
          <div key={i} className="particle" style={{
            ...
            '--dur': `${3 + Math.random() * 6}s`, '--del': `${Math.random() * 6}s`,
            '--max-op': 0.15 + Math.random() * 0.5,
          } as any} />
        ))}
      </div>

      {/* ✅ Header TOUJOURS rendu — z-index 300 par-dessus le disclaimer aussi */}
      ...
      <div className="main-content">
        <div className="orb-container" style={{ marginBottom: 48 }}>
          ...
          <div className="orb-dot"><div className="orb-dot-inner" /></div>
        </div>
