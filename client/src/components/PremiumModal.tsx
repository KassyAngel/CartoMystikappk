...
export default function PremiumModal({ isOpen, onClose, onPurchase }: PremiumModalProps) {
  ...
  const loadRevenueCatOfferings = async () => {
    try {
      ...
      if (!availableOfferings || !availableOfferings.current) {
        console.error('❌ Aucune offre "current" trouvée');
      ...
      setError('Impossible de charger les offres. Vérifiez votre configuration RevenueCat.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative bg-gradient-to-br from-purple-900 via-indigo-900 to-purple-800 rounded-2xl shadow-2xl max-w-md w-full p-6 border-2 border-yellow-500/30 max-h-[90vh] overflow-y-auto">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-purple-300 text-xs mt-2">
            {isNative ? '📱 Paiement Google Play' : '🌐 Paiement Web (Stripe)'}
          </p>
        </div>
        ...
        {isNative && !showRestoreForm && (
          <>
            {isLoading ? (
              ...
              <div className="text-center py-8">
                <p className="text-purple-200 mb-4">Aucune offre disponible</p>
                <button
                  onClick={loadRevenueCatOfferings}
        ...
        <div className="mt-3 flex items-center justify-center gap-2 text-purple-300 text-xs">
          ...
          {isNative ? (
            <span className="font-semibold text-yellow-300">Google Play</span>
          ) : (
